...
filtered_data <- cleaned_data %>%
  filter(vehicle_brand %in% top_brands, !is.na(price_num), price_num > 1000) %>%
  mutate(vehicle_brand = factor(vehicle_brand, levels = rev(top_brands)))  # Reversed order

ggplot(filtered_data, aes(x = vehicle_brand, y = price_num)) +
  geom_boxplot(fill = "darkred", color = "black", alpha = 0.7, outlier.color = "red", outlier.shape = 16) +
  coord_flip() +
  scale_y_continuous(labels = scales::comma) +
  labs(title = "Price Distribution of Top 10 Truck Brands",
       x = "Brand",
       y = "Price (PLN)") +
  theme_minimal()
```
